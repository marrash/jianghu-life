(function(root){
  function extend(L){
    ...
    function die(s){const d=integer(s,1,6);return s.cheat?6:s.traits.steady?Math.max(2,d):d;}
    function log(s,type,title,text,effect={}){s.history.push({year:s.year,age:L.age(s),type,title,text,effect});}
    function apply(s,e){const actual={};for(const[k,v]of Object.entries(e)){const n=clamp(s[k]+v,0,k==='money'?9999:100);actual[k]=n-s[k];s[k]=n;if(keys.includes(k)&&s.carry)s.carry[k]=Math.min(s.carry[k],cost(s,k)-1);}return actual;}
    function cost(s,k){return (s[k]>=85?3:s[k]>=60?2:1)*(s[k]>=s.potential[k]?3:1);}
    function gain(s,k,points){const old=s[k];s.carry[k]+=points;while(s[k]<100&&s.carry[k]>=cost(s,k)){s.carry[k]-=cost(s,k);s[k]++;}if(s[k]===100)s.carry[k]=0;return s[k]-old;}
    ...
    function rollTraining(state){if(state.phase!=='action'||state.training.rolled)throw Error('此章已擲骰，請分配修行成果');const s=copy(state);let n=integer(s,3,6);if(s.health<35)n=Math.max(2,n-1);s.training.rolled=true;s.training.dice=Array.from({length:n},()=>die(s));s.training.base={stats:Object.fromEntries(keys.map(k=>[k,s[k]])),carry:copy(s.carry)};return s;}
    function allocate(state,k){if(state.phase!=='action'||!state.training.rolled||!keys.includes(k)||state[k]>=100||state.training.index>=state.training.dice.length)throw Error('目前不能分配這顆骰子');const s=copy(state);gain(s,k,s.training.dice[s.training.index]);s.training.picks.push(k);s.training.index++;return s;}
    ...
    function finishTraining(state){if(state.phase!=='action'||!state.training.rolled||state.training.index!==state.training.dice.length)throw Error('請先分配所有骰子');const s=copy(state),changes={};for(const k of keys){changes[k]=s[k]-s.training.base.stats[k];s.focus[k]+=s.training.picks.filter(x=>x===k).length;}log(s,'training','修行有得',`本章擲出 ${s.training.dice.join('、')}；已分配 ${s.training.dice.length} 顆修行骰。`,changes);if(!s.traits.focused&&Math.max(...Object.values(s.focus))>=12){s.traits.focused=true;log(s,'trait','專精一道','長期投入同一門功夫，比武實力獲得額外 4 點。');}s.phase='event';return s;}
    ...
    function resolveDuel(state,style){if(state.phase!=='battle'||!['safe','balanced','bold'].includes(style))throw Error('目前不能比武');const s=copy(state),duel=L.duelInfo[s.year],base=Math.round(s.skill*.4+s.inner*.25+s.body*.15+s.agility*.1+s.insight*.1)+(s.traits.focused?4:0),health=-Math.floor((100-s.health)/10);let wins=0;const logs=[];
